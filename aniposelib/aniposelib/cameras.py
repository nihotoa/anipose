...
class Camera:
    ...
    def reprojection_error(self, p3d, p2d):
        ...
        引数の説明
        self: 任意のカメラオブジェクト.
        p3d: 求められた3次元推定値, フレームとポイントを混在したインデックスの数だけデータがある。
        p2d: 正規化された実際の画像座標.使用するカメラオブジェクトによって変わる, フレームとポイントを混在したインデックスの数だけデータがある。
        projはp3dとselfの情報をもとに、p3dを基準平面に投射した推定平面座標値であり、フレームとポイントを混在したインデックスの数だけデータがある。
...
class CameraGroup:
    ...
    def triangulate(self, points, undistort=True, progress=False):
        ...
        if len(points.shape) == 2:
            points = points.reshape(-1, 1, 2)
            one_point = True
        # 歪みを補正し、正規化された画像座標を返す(x,y座標共にとりうる値の範囲は[-1, 1]であり、[x,y] = [0,0]が画像中心)
        if undistort:
        ...
        # 基準座標(一番最初のカメラのカメラ座標系)から各カメラのカメラ座標系に対する座標変換行列を取得する(作成に必要な情報は各カメラのインスタンスが持っている)
        ...
        # n_points(フレームとポイントの情報が混在しているindex)をiteration
        for ip in iterator:
            # ipにおける全カメラからの正規化された画像座標を取得
            subp = points[:, ip, :]
    ...
    def triangulate_possible(self, points, undistort=True,
        ...
        # フレーム数 * キーポイント数のiterationのiteration
        for point_ix in iterator:
            ...
            # point_ixの３次元推定に使用可能なカメラの組み合わせの情報を抽出する(pickedに入る)
            for picked in itertools.product(*all_iters[point_ix].values()):
                picked = [p for p in picked if p is not None]
                # 使用するカメラが３台より少ない & 使用できるカメラすべてを使用していないなら、そのカメラの組み合わせは論ずるに値しない
                if len(picked) < min_cams and len(picked) != n_cams_max:
                    continue
                
                # pickedを使用して、使用できるカメラのidxをリストにする(xnumsは特に意味ない。よって必要なのはpickedの各tupleの0番目の値)
                cnums = [p[0] for p in picked]
                xnums = [p[1] for p in picked]

                # point_ix(フレーム数とキーポイントの両方を加味したインデックス)におけるcnumsに該当するのカメラからの画像座標をptsにまとめる
                pts = points[cnums, point_ix, xnums]
                cc = self.subset_cameras(cnums)

                # シンプルにtriangulationを行う、そのあと、再投影誤差を返す(point_idxの一点のみなので、返される３次元座標値も再投影誤差も1つだけ)
                p3d = cc.triangulate(pts, undistort=undistort)
                err = cc.reprojection_error(p3d, pts, mean=True)
                
                # 試したペアの中で一番再投影誤差が小さくなった場合、辞書を更新する
                if err < best_error:
                    best_point = {
                        'error': err,
                        'point': p3d[:3],
            ...
            # point_ixの点において、もっとも再投影誤差が小さくなった時の３次元推定値と、使用したカメラの組み合わせの情報等をまとめる
    ...
    def bundle_adjust_iter(self, p2ds, extra=None,
        ...
        # 全フレーム全ポイントにおける画像座標の配列と、その他の情報の配列(各カメラから基準座標への回転ベクトル、並進ベクトル、3次元座標値)を受け取る
        p2ds_full = p2ds
        extra_full = extra
        
        # 何らかの基準をもとに使用するデータ(あるフレームにおけるあるポイントのデータ)をフィルタリングする
        p2ds, extra = resample_points(p2ds_full, extra_full,
                                      n_samp=n_samp_full)
        
        # 全画像座標と現在のカメラパラメータを用いて３次元座標を推定し、その、後各カメラの平面に投影し、そのカメラからの実際の画像座標との誤差を求める
        # 簡潔に言うと現在持っているすべての画像座標と、そこから求められた3次元座標から再投影誤差を求めて、その中央値を返す
        # p2dsは各カメラからの画像座標だが、フレームとポイントの両方が混在している
        ...
        # 再投影誤差を小さくするように外部パラメータ行列を変えていく(extraの中の回転ベクトルと並進ベクトル)
        for i in range(n_iters):
            # 何かしらのフィルタによって使用する画像座標と対応するデータを絞り込む
            p2ds, extra = resample_points(p2ds_full, extra_full,
                                          n_samp=n_samp_full)
            ...
            # 3次元推定の使用したカメラのペアと、そのカメラのペアからの再投影誤差の最大値と最小値を使用して辞書を作成
    ...
    def set_rotations(self, rvecs):
        for cam, rvec in zip(self.cameras, rvecs):
            # 指定したカメラインスタンスのインスタンス変数rvecにrvecを代入する
    ...
    def average_error(self, p2ds, median=False):
        # 画像座標と現在の座標変換行列を用いて全フレームの3次元座標を推定
        p3ds = self.triangulate(p2ds)

        # 全フレームの推定された3次元座標を各カメラの平面座標に投影し、すべてのフレームにおける再投影誤差を求める
        # 返り値の長さはフレーム数に等しい(本来はフレーム数 * カメラ数だが、カメラ数の分は同じフレーム同士のmean)
        errors = self.reprojection_error(p3ds, p2ds, mean=True)

        # 全フレームからの投影誤差を求め、その中央値を返す
        if median:
    ...
    def calibrate_rows(self, all_rows, board,
        ...
        for rows, camera in zip(all_rows, self.cameras):
            ...
            # 内部パラメータの初期値を計算(matrixに格納).
            # 全フレームにおける、チェッカーボードの全キーポイントの3D座標値と画像座標値から内部パラメータを計算する
            if init_intrinsics:
        ...
        # 各カメラのインスタンスのプロパティをprint
        print(self.get_dicts())

        # 各フレームのcharucoボードの3次元座標 -> 各カメラのカメラ座標に対する回転ベクトルrvecと並進ベクトルを計算
        ...
        # すべてのフレームにおけるすべてのキーポイントを調べて、2台以上のカメラから追えてなかった（画像座標を取得できな買った）場合はそのフレームにおけるそのチェックポイントの情報を削除
        # ２台以上のカメラから追えているチェックポイントの画像座標と、それに対応する情報(3D-coordination, ids, etc...)を返す
        imgp, extra = extract_points(merged, board, min_cameras=2)

        # 外部パラメータの初期値を計算
        if init_extrinsics:
            # rvecsとtvecsを連結して6次元のベクトルを作る
            rtvecs = extract_rtvecs(merged)
            if verbose:
                # カメラ同士の組み合わせと、その組み合わせにおけるボードを同時に撮影できた総フレーム数をprint
                pprint(get_connections(rtvecs, self.get_names()))

            # 最初のカメラのカメラ座標系を基準の３次元座標とし、そのカメラからほかのカメラへの変換行列を求める。それを各カメラの外部パラメータ行列の初期値とする
            # rvecs, tvecsは、基準カメラから各カメラへ座標変換する際の回転ベクトル(オイラー角)と並進ベクトルをまとめたもの
            rvecs, tvecs = get_initial_extrinsics(rtvecs, self.get_names())

            # 各カメラのインスタンス変数rvecs,tvecsに,rvecsとtvecsを代入する
    ...
    def get_rows_videos(self, videos, board, verbose=True):
        ...
        for cix, (cam, cam_videos) in enumerate(zip(self.cameras, videos)):
            ...
            for vnum, vidname in enumerate(cam_videos):
                ...
                # videosで指定したキャリブレーション動画の全フレームの全コーナーを検出して結果を辞書にしてrowsに返す
